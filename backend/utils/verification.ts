 ...
 */
export const getVerificationCode = async (verificationId: string): Promise<string | null> => {
  const key = `verification:${verificationId}`;
 ...
 * @param verificationId - The unique verification ID
 */
export const removeVerificationCode = async (verificationId: string): Promise<void> => {
  const key = `verification:${verificationId}`;
  await redis.del(key);
};
