...
// Store OTPs with expiration (now uses Redis)
const OTP_EXPIRATION = 10 * 60; // 10 minutes in seconds
/**
 * Request OTP
 * This function handles the OTP request from the user.
...
// Send phone verification code - updated for better error handling
export const sendPhoneVerificationCode = async (req, res) => {
    try {
        ...
        // Client-side Firebase authentication guidance for production
        ...
        // For development/testing, continue with server-side OTP
        ...
        const smsSent = await sendSMS(phoneNumber, `Your Radeo Shop verification code is: ${verificationCode}. It expires in 10 minutes.`);
        if (smsSent) {
            res.json({
...
// Verify Firebase phone auth token and handle user
export const verifyFirebasePhoneAuth = async (req, res) => {
    try {
        ...
        if (!user) {
            user = await User.create({
                ...
                isPhoneVerified: true,
                role: 'user',
                // Add a dummy password for now - user can update later
